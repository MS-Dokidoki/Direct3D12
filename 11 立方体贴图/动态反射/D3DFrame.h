...
private:
	void BuildCubeDescriptors();
	void BuildCubeCameras();
	void UpdateCubeScenes(const GameTimer&);
	void DrawToCube();

private:
	CubeRenderTarget CubeRTarget;
	
	UINT nCubeSize = 512;
	CD3DX12_CPU_DESCRIPTOR_HANDLE hCubeRenderTargetDSV;
	Camera CubeCameras[6];
    Microsoft::WRL::ComPtr<ID3D12Resource> pCubeDepthStencilBuffer;
