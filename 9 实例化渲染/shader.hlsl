...
struct PsInput
{
    ...
    nointerpolation uint iMatIndex: MATINDEX;
};

SamplerState gsamAnisotropicWrap: register(s0);

Texture2D DiffuseTextures[5]: register(t0);
StructuredBuffer<MaterialData> MaterialDatas: register(t0, space1);
StructuredBuffer<InstanceData> InstanceDatas: register(t1, space1);

PsInput VsMain(VsInput input, uint instanceID: SV_INSTANCEID)
{
    PsInput output;
    InstanceData insData = InstanceDatas[instanceID];
    MaterialData matData = MaterialDatas[insData.nMaterialIndex];

    float4 posW = mul(float4(input.iPos, 1.0), insData.matWorld);
    output.iPosW = posW.xyz;

    output.iPos = mul(posW, matViewProj);
    output.iNormal = mul(input.iNormal, (float3x3)insData.matWorld);

    float4 texC = mul(float4(input.iTexC, 0.0, 1.0), insData.matTexTransform);
    output.iTexC = mul(texC, matData.matMaterialTransform).xy;

    output.iMatIndex = insData.nMaterialIndex;
    return output;
}
...
float4 PsMain(PsInput input): SV_TARGET
{
    ...
    MaterialData matData = MaterialDatas[input.iMatIndex];
