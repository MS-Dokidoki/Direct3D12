...
void D3DApp::OnResize()
{
    ...
    for(UINT i = 0; i < sc_cSwapChainBuffer; ++i)
        ppSwapChainBuffer[i].Reset();
    pDepthStencilBuffer.Reset();

    ThrowIfFailed(pSwapChain->ResizeBuffers(
        sc_cSwapChainBuffer, cxClient, cyClient, emBackBufferFormat, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH
    ));
    ...
    stScissorRect.right = cxClient;
    stScissorRect.bottom = cyClient;
...
void D3DApp::CreateSwapChain()
{
    ...
    scd.BufferCount = sc_cSwapChainBuffer;
...
void D3DApp::CreateDescriptor()
{
    ...
    heapDesc.NumDescriptors = sc_cSwapChainBuffer;
    heapDesc.NodeMask = 0;
    ...
    nCSUDescriptorSize = pD3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
