...
UINT D3DHelper::CalcConstantBufferBytesSize(UINT size)
{
    return (size + 255) & ~255;
}
...
Microsoft::WRL::ComPtr<ID3D12Resource> D3DHelper::CreateDefaultBuffer(ID3D12Device* pDevice, ID3D12GraphicsCommandList* pComList, 
    const void* pData, UINT uiByteSize, ID3D12Resource** pUploader)
{
    ...
    void* pBegin;
    ThrowIfFailed((*pUploader)->Map(0, NULL, &pBegin));
    memcpy(pBegin, pData, uiByteSize);
