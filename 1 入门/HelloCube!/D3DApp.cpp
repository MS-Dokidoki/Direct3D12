...
{
    return pCBVHeap->GetCPUDescriptorHandleForHeapStart();
}
void D3DApp::OnResize()
{
    ...
    pSwapChain->ResizeBuffers(sc_iSwapChainBufferCount, cxClient, cyClient, emBackBufferFormat, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH);
}
...
void D3DApp::CreateDescriptor()
{
    ...
    ThrowIfFailed(pD3dDevice->CreateDescriptorHeap(
        &heapDesc, IID_PPV_ARGS(&pCBVHeap)
    ));

    D3D12_RESOURCE_DESC descDS;
    descDS.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
    descDS.Alignment = 0;
    descDS.Width = cxClient;
    descDS.Height = cyClient;
    descDS.DepthOrArraySize = 1;
    descDS.MipLevels = 1;
    descDS.Format = emDepthStencilFormat;
    descDS.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
    descDS.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
    ...
    pD3dDevice->CreateDepthStencilView(pDepthStencilBuffer.Get(), NULL, pCBVHeap->GetCPUDescriptorHandleForHeapStart());
}
