...
private:
    ...
    UINT nCBMaterialByteSize = D3DHelper_CalcConstantBufferBytesSize(sizeof(Light::MaterialConstant));
...
/**** */
    ...
    pSlotParams[2].InitAsConstantBufferView(2);
    descRS.Init(3, pSlotParams, 0, NULL, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
...
//
    ...
    for(UINT i = 0; i < RenderItems.size(); ++i)
    {
        ...
        addr += nCBMaterialByteSize * item.pMaterial->nCBIndex;
...
void D3DFrame::OnKeyboardInput(const GameTimer& t)
{
...
{
    Light::Material water;
    Light::Material grass;
    grass.Name = "grass";
    grass.nCBIndex = 0;
    grass.vec4DiffuseAlbedo = XMFLOAT4(0.2f, 0.6f, 0.2f, 1.0f);
    grass.vec3FresnelR0 = XMFLOAT3(0.01, 0.01, 0.01f);
    grass.nRoughness = 0.125f;
    grass.iNumFramesDirty = 3;

    water.Name = "water";
    water.nCBIndex = 1;
    water.vec4DiffuseAlbedo = XMFLOAT4(0.0f, 0.2f, 0.6f, 1.0f);
    water.vec3FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
    water.nRoughness = 0.0f;
    water.iNumFramesDirty = 3;
    ...
    for(UINT i = 0; i < nFrameResourceCount; ++i)
        pFrameResources[i].InitOtherConstantBuffer(pD3dDevice.Get(), Render::FrameResource::CBTYPE_MATERIAL, 0, 2/*Materials.size()*/);
}
