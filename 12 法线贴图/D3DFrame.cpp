...
void D3DFrame::BuildRenderItems()
{
    ...
    for(UINT i = 1; i < AllRenderItems.size(); ++i)
...
void D3DFrame::BuildRootSignatures()
{
    ...
    range[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, Textures.size() - 1, 1);
...
void D3DFrame::BuildSRV()
{
    ...
    // 5
    res = Textures["bricks_normal"].pResource.Get();
    srvDesc.Format = res->GetDesc().Format;
    srvDesc.Texture2D.MipLevels = res->GetDesc().MipLevels;
    pD3dDevice->CreateShaderResourceView(res, &srvDesc, handle);
    handle.Offset(1, nCbvSrvUavDescriptorSize);
    ...
    // 6
    res = Textures["default_normal"].pResource.Get();
    srvDesc.Format = res->GetDesc().Format;
    srvDesc.Texture2D.MipLevels = res->GetDesc().MipLevels;
    pD3dDevice->CreateShaderResourceView(res, &srvDesc, handle);
    handle.Offset(1, nCbvSrvUavDescriptorSize);
    ...
    // 7
    res = Textures["wood_normal"].pResource.Get();
    srvDesc.Format = res->GetDesc().Format;
    srvDesc.Texture2D.MipLevels = res->GetDesc().MipLevels;
    pD3dDevice->CreateShaderResourceView(res, &srvDesc, handle);
    handle.Offset(1, nCbvSrvUavDescriptorSize);
...
void D3DFrame::Render(const GameTimer& t)
{
    ...
    pCommandList->SetGraphicsRootDescriptorTable(3, CD3DX12_GPU_DESCRIPTOR_HANDLE(pSRVHeap->GetGPUDescriptorHandleForHeapStart(), nCbvSrvUavDescriptorSize, Textures.size() - 1));
