...
bool D3DFrame::Initialize()
{
    ...
    ssao.Init(pD3dDevice.Get(), pCommandList.Get(), cxClient, cyClient);
    ...
    ssao.CalcGaussWeights(2.5f, weights);
    CopyMemory(this->weights, &weights[0], 12 * sizeof(float));
...
void D3DFrame::BuildRootSignatures()
{
    for(UINT i = 0; i < nFrameResourceCount; ++i)
    {
        ...
        pFrameResources[i].InitOtherBuffer(Resource::FrameResource::FRAME_RESOURCE_TYPE_SSAO).Init(pD3dDevice.Get(), CONSTANT_VALUE::nCBSsaoByteSize, 1);
    }
    ...
    range[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 14, 3, 0);
...
#define CreatePipelineState(type, desc) ThrowIfFailed(pD3dDevice->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&pPipelineState))); PipelineStates[type] = pPipelineState
    ...
    opaque.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_EQUAL;
    ...
    this->ssao.SetPipelineState(PipelineStates[RENDER_TYPE_SSAO].Get(), PipelineStates[RENDER_TYPE_SSAO_BLUR].Get());
}

void D3DFrame::BuildSRV()
{
    D3D12_DESCRIPTOR_HEAP_DESC desc;
    desc.NumDescriptors = 8 + 1 + 1 + 2 + 3 + 3;
    ...
    nNormalMapSrvIndex = nShadowMapSrvIndex + 3;
    nNullMapSrvIndex = nNormalMapSrvIndex + 3;
	
    // CubeMap
    CD3DX12_CPU_DESCRIPTOR_HANDLE handle(pSRVHeap->GetCPUDescriptorHandleForHeapStart(), nSkyMapSrvIndex, nCbvSrvUavDescriptorSize);
    ...
    handle.Offset(nNullMapSrvIndex - nSkyMapSrvIndex, nCbvSrvUavDescriptorSize);
    ...
    pD3dDevice->CreateShaderResourceView(NULL, &srvDesc, handle);
    ...
    ssao.BuildDescriptors(
        ...
        CD3DX12_CPU_DESCRIPTOR_HANDLE(pSRVHeap->GetCPUDescriptorHandleForHeapStart(), nNormalMapSrvIndex - 2, nCbvSrvUavDescriptorSize),
        CD3DX12_GPU_DESCRIPTOR_HANDLE(pSRVHeap->GetGPUDescriptorHandleForHeapStart(), nNormalMapSrvIndex - 2, nCbvSrvUavDescriptorSize),
        CD3DX12_CPU_DESCRIPTOR_HANDLE(pRTVHeap->GetCPUDescriptorHandleForHeapStart(), nSwapChainBufferCount, nRTVDescriptorSize),
...
void D3DFrame::CreateDescriptor()
{
    ...
    heapDesc.NumDescriptors = nSwapChainBufferCount + 3;
    heapDesc.NodeMask = 0;
...
void D3DFrame::DefineSkullAnimation()
{
    ...
    SkullAnimation.Keyframes[0].nTimePos = 0.0f;
    SkullAnimation.Keyframes[0].vec3Translation = {-7.0f, 0.0f, 0.0f};
    SkullAnimation.Keyframes[0].vec3Scale = {0.25f, 0.25f, 0.25f};
    XMStoreFloat4(&SkullAnimation.Keyframes[0].vec4RotationQuat, q0);

    SkullAnimation.Keyframes[1].nTimePos = 2.0f;
    SkullAnimation.Keyframes[1].vec3Translation = {0.0f, 2.0f, 10.0f};
    SkullAnimation.Keyframes[1].vec3Scale = {0.5f, 0.5f, 0.5f};
    XMStoreFloat4(&SkullAnimation.Keyframes[1].vec4RotationQuat, q1);

    SkullAnimation.Keyframes[2].nTimePos = 4.0f;
    SkullAnimation.Keyframes[2].vec3Translation = {7.0f, 0.0f, 0.0f};
    SkullAnimation.Keyframes[2].vec3Scale = {0.25f, 0.25f, 0.25f};
    XMStoreFloat4(&SkullAnimation.Keyframes[2].vec4RotationQuat, q2);
    
    SkullAnimation.Keyframes[3].nTimePos = 6.0f;
    SkullAnimation.Keyframes[3].vec3Translation = {0.0f, 1.0f, -10.0f};
    SkullAnimation.Keyframes[3].vec3Scale = {0.5f, 0.5f, 0.5f};
    XMStoreFloat4(&SkullAnimation.Keyframes[3].vec4RotationQuat, q3);

    SkullAnimation.Keyframes[4].nTimePos = 8.0f;
    SkullAnimation.Keyframes[4].vec3Translation = {-7.0f, 0.0f, 0.0f};
    SkullAnimation.Keyframes[4].vec3Scale = {0.25f, 0.25f, 0.25f};
    XMStoreFloat4(&SkullAnimation.Keyframes[4].vec4RotationQuat, q0);
}
...
void D3DFrame::UpdateAnimation(float dt)
{
    nAnimTimePos += dt;

    if(nAnimTimePos >= SkullAnimation.GetEndTime())
        nAnimTimePos = 0.0f;
    
    RenderItem& skull = AllRenderItems[nSkullRenderItemIndex];
    SkullAnimation.Interpolate(nAnimTimePos, skull.matWorld);
    skull.iFramesDirty = 3;
...
void D3DFrame::Render(const GameTimer& t)
{
    ...
    DrawSceneToNormalMap();

    pCommandList->SetGraphicsRootSignature(pSsaoRootSignature.Get());
    ssao.ComputeSsao(pCommandList.Get(), pCurrFrameResource->CBOthers[Resource::FrameResource::FRAME_RESOURCE_TYPE_SSAO].Resource()->GetGPUVirtualAddress(), 0);
    pCommandList->SetGraphicsRootSignature(pRootSignature.Get());
